...
class XmlCodecTest extends \PHPUnit_Framework_TestCase {
  ...
  public function testSerializeSimpleMessage() {
    ...
    $this->assertEquals((int)$sxe->int32, 1000);
  }
  ...
  public function testSerializeRepeatedFields() {
    ...
    $this->assertEquals($xml->string[0], 'one');
    $this->assertEquals($xml->string[1], 'two');
    $this->assertEquals($xml->string[2], 'three');
    ...
    $xml = Protobuf::encode($repeated);
    $xml = simplexml_load_string($xml);
    $this->assertEquals((int)$xml->int[0], 1);
    $this->assertEquals((int)$xml->int[1], 2);
    $this->assertEquals((int)$xml->int[2], 3);
